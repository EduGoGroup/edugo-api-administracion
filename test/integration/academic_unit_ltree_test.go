...
// STUB_FASE2: Estos tests requieren PostgreSQL con extensión ltree
...
func TestAcademicUnitRepository_FindChildren(t *testing.T) {
	t.Skip("STUB_FASE2: Requiere PostgreSQL con ltree extension - Completar en FASE 2 (Claude Local)")
...
func TestAcademicUnitRepository_FindByPath(t *testing.T) {
	...
	// require.NoError(t, repo.Create(ctx, grade))
	//
	// // El path debería ser solo el UUID del grado (es raíz)
	// expectedPath := grade.ID().String()
	//
...
func TestAcademicUnitRepository_MoveSubtree(t *testing.T) {
	...
	// err := repo.MoveSubtree(ctx, section.ID(), &grade2.ID())
	// require.NoError(t, err)
	//
...
// =====================================================
// Helper functions (TO BE IMPLEMENTED IN FASE 2)
// =====================================================

// TODO_FASE2: Implementar helper para crear escuela de test
// func createTestSchool(t *testing.T, db *sql.DB, name, code string) *entity.School {
//     schoolID := valueobject.NewSchoolID()
//     query := `INSERT INTO schools (id, name, code, created_at, updated_at) VALUES ($1, $2, $3, NOW(), NOW())`
//     _, err := db.Exec(query, schoolID.String(), name, code)
//     require.NoError(t, err)
//     // ... reconstruct and return school entity
// }
